...
echo   Junior Coder  ^| Port 8080  (Qwen 2.5 Coder 1.5B)
echo   Senior Reasoner ^| Port 8081 (DeepSeek Coder 1.3B)
echo  ============================================================
echo.

:: ----------------------------------------------------------------
:: STEP 1 - Start Junior Coder server (port 8080) in background
:: ----------------------------------------------------------------
echo [1/4] Starting Junior Coder server on port 8080...
start "Qwen Coder Server - Junior" /MIN cmd /c ^
    "cd /d C:\AI\llama.cpp && llama-server.exe -m C:\AI\llama.cpp\models\qwen2.5-coder-1.5b-instruct-q4_k_m.gguf -c 4096 --port 8080 --host 127.0.0.1"

:: ----------------------------------------------------------------
:: STEP 2 - Start Senior Reasoner server (port 8081) in background
:: ----------------------------------------------------------------
echo [2/4] Starting Senior Reasoner server on port 8081...
start "DeepSeek R1 Server - Senior" /MIN cmd /c ^
    "cd /d C:\AI\llama.cpp && llama-server.exe -m C:\AI\llama.cpp\models\deepseek-coder-1.3b-instruct.Q4_K_M.gguf -c 8192 --port 8081 --host 127.0.0.1"
...
echo       (Polling ports 8080 and 8081 - up to 120 seconds)
echo.
...
:WAIT_LOOP
    ...
    if %ATTEMPTS% GTR %MAX_ATTEMPTS% (
        echo.
        echo [ERROR] Servers did not become ready within 120 seconds.
        echo.
        echo   Diagnostics:
        ...
        echo.
        echo   Check the minimised server windows for error messages.
        pause
        exit /b 1
    )

    :: Check port 8080 - Junior Coder (Qwen 2.5 Coder 1.5B)
    netstat -an | findstr ":8080 " | findstr "LISTENING" >nul 2>&1
    set JUNIOR_OK=%ERRORLEVEL%

    :: Check port 8081 - Senior Reasoner (DeepSeek Coder 1.3B)
    netstat -an | findstr ":8081 " | findstr "LISTENING" >nul 2>&1
...
echo   Server windows remain open in the taskbar.
